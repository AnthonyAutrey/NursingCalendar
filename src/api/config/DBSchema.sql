...
(
	...
	FOREIGN KEY (RoomName) REFERENCES Rooms(RoomName) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (LocationName) REFERENCES Locations(LocationName) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (CWID) REFERENCES Users(CWID) ON UPDATE CASCADE ON DELETE CASCADE
);
...
(
	...
	ToCWID INT NOT NULL,
	PRIMARY KEY (NotificationID), FOREIGN KEY (FromCWID) REFERENCES Users(CWID) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (ToCWID) REFERENCES Users (CWID) ON UPDATE CASCADE ON DELETE CASCADE
);
	...
	(
		...
		(
			...
			FOREIGN KEY (LocationName) REFERENCES Locations(LocationName) ON UPDATE CASCADE ON DELETE CASCADE,
			FOREIGN KEY (RoomName) REFERENCES Rooms(RoomName) ON UPDATE CASCADE ON DELETE CASCADE,
			FOREIGN KEY (ResourceName) REFERENCES Resources(ResourceName) ON UPDATE CASCADE ON DELETE CASCADE
		);
...
(
	...
	WeeklyDays VARCHAR
		(7), -- use single character identifiers: (mtwrfsu), if doesn't recur weekly, then NULL
	PRIMARY KEY
		(RecurringID, EventID, LocationName, RoomName),
	FOREIGN KEY
		(EventID, LocationName, RoomName) REFERENCES Events
		(EventID, LocationName, RoomName)
);
...
----------------------------------------------*/
		...
		INSERT INTO Users
			(CWID, FirstName, LastName, UserRole)
		VALUES
	...
	(99999999, 'Bran', 'Stark', 'student'); */
		...
		INSERT INTO Rooms
			(RoomName, Capacity, LocationName)
		VALUES
		...
		INSERT INTO RoomResourceRelation
			(LocationName, RoomName, ResourceName, Count)
		VALUES
...
VALUES
	...
	(99999999, 'Dummy User: Bran Stark', 'Bran is a semester 1 student'); */
		...
		INSERT INTO Events
			(EventID, LocationName, RoomName, Title, Description, StartTime, EndTime, CWID)
		VALUES
		...
		INSERT INTO EventGroupRelation
			(EventID, LocationName, RoomName, GroupName)
		VALUES
